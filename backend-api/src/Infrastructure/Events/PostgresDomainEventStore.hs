...
generatePlaceholders n = Data.Text.concat ["(", replicateWithComma n "?", ")"]

replicateWithComma :: Int -> Text -> Text
replicateWithComma n s = intercalate ", " (replicate n s)
  where
    replicate n s = map (const s) [1 .. n]
